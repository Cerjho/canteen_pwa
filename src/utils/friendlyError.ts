...
const ERROR_MAP: Array<{ pattern: RegExp; friendly: string }> = [
  ...
  { pattern: /ORDER_LOCKED|order.*locked/i, friendly: 'This order is already being prepared and cannot be modified. Please wait until it is completed.' },
  { pattern: /MERGE_FAILED|merge.*failed/i, friendly: 'Failed to add items to the existing order. Please try again.' },
