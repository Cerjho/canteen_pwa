...
// Warm the offline page into the runtime cache on install
self.addEventListener('install', (_ev) => {
  console.log('[SW] Installing service worker...');
  // Store in a dedicated runtime cache so setCatchHandler can find it
  _ev.waitUntil(
...
// Catch handler: serves offline page for failed navigation requests
setCatchHandler(async ({ request }) => {
  ...
  // For non-document requests (images, API, etc.), just return a network error
...
// Uses Web Locks API to prevent concurrent processing with main thread
async function syncQueuedOrders(): Promise<void> {
  if (navigator.locks) {
    return navigator.locks.request(
      ...
      async (lock) => {
        if (!lock) {
          console.log('[SW] Queue processing lock held by another context, skipping');
          return;
        }
        return syncQueuedOrdersInner();
      }
    );
  }
  return syncQueuedOrdersInner();
...
async function syncQueuedOrdersInner(): Promise<void> {
  ...
  event.waitUntil(
    Promise.all([
      ...
      caches.keys().then((cacheNames) => {
        return Promise.all(
          cacheNames
            .filter((name) => !name.includes('precache') && name !== 'offline-fallback')
            .map((name) => {
