...
| --------- | -------- | --------- |
| Cash payment | âœ… Full | 4-hour timeout, staff confirmation, auto-cleanup, wallet refund on timeout |
...
- `payment_status` enum: `awaiting_payment | paid | timeout | refunded | failed` âœ…
- `payment_due_at` timestamp for deadlines âœ…
- `payment_group_id` UUID for batch order tracking âœ…
- Atomic `increment_stock`/`decrement_stock` RPCs with row locks âœ…
- Order status transition validation trigger âœ…
